...
cs1300bmp_readfile(char *filename, struct cs1300bmp *image)
{
  ...
  if ( error ) {
	...
	image -> color[0  ][row][col] = rarray[row * width + col];
	image -> color[1][row][col] = garray[row * width + col];
	image -> color[2 ][row][col] = barray[row * width + col];
      }
    }
    //
    //  Free the memory.
...
cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  for (short row = 0; row < height; row ++ ) {
    for (short col = 0; col < width; col ++ ) {
      rarray[row * width + col] = image -> color[0  ][row][col];
      garray[row * width + col] = image -> color[1][row][col];
      barray[row * width + col] = image -> color[2 ][row][col];
    }
  }
